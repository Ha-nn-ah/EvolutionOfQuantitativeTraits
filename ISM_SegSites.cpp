...
class Generation{
    public:
        ...
        double Stop[2]; // stopping condition (number of generations the population is evolving)
        double ZMean; // mean phenotype in the population
        int t;
    ...
    void Initialization (double pop_size,double sel,double mutation_rate,int mut_effect,double mean_effect,double tau[2],int selected_seed); // Initialise the random number generator from the GNU-Library. We choose the MT19937 generator. Intitialise arrays for population, age and fitness.
        bool Evolve (void);
        void Mutation (void);
        void Reproduction (gsl_rng *rng);
};

void Generation::Initialization(double pop_size,double sel,double mutation_rate,int mut_effect,double mean_effect,double tau[2],int selected_seed){
...
bool Generation::Evolve(void){
    ...
    if (Stop[1] == 0) { // stopping condition: number of generations
        while (t<Stop[0]){ // generation 1 to StopCond
            t++;
            seed++;
            Reproduction(rng);
        }
    } else if (Stop[1] == 1){ // stopping condition: mean phenotype reached in population
        while (ZMean < Stop[0]){ // until mean phenotype Stop reached in population
            t++;
            seed++;
            Reproduction(rng);
        }
...
int main(){
    ...
    double StopCond[2] = {100,0}; // stopping condition (2nd entry: 0 = number of generations, 1 = mean phenotype)
    ...
    for (int j=0;j<NumR;j++){
        ...
        if(!gen.Evolve()){
            ...
            myfile << gen.t;  // number of generations needed to reach the stopping condition for each run
            myfile << "\t" << count(gen.popArray.begin(), gen.popArray.end(), 0); // number of lost mutants
            myfile << "\t" << gen.ZMean; // mean phenotype in the population at the end of each run
            for(int i=0;i<gen.popArray.size(); i++){
                if (gen.popArray[i] != 0) { // mutant frequency of fixed mutants and on segregating sites ordered by their appearance
